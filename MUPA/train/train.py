...
from MUPA.constants import REG_TOKEN, SEG_S_TOKEN, SEG_E_TOKEN
from MUPA.dataset import HybridDataCollator, HybridDataset
from MUPA.model import MODELS
from MUPA.model.builder import build_model
from MUPA.train.custom_trainer import CustomTrainer
...
class ModelArguments:
    ...
    use_construct: Optional[bool] = field(default=False)
    use_average: Optional[bool] = field(default=False)
    use_duration: Optional[bool] = field(default=False)
...
class DataArguments:
    ...
    use_qa: Optional[bool] = field(default=False)
...
def train(TrainingArguments, Trainer):
    ...
    assert model_args.role in ('all_in_one', 'planner', 'grounder', 'verifier', 'answerer', 'GQA')
    ...
    if training_args.lora_enable and not isinstance(model, PeftModel):
        ...
        tune_lm_head = model.config.role in ('all_in_one', 'grounder', 'verifier', 'GQA')
    ...
    for n, p in model.named_parameters():
        ...
        if model_args.role in ('all_in_one', 'grounder', 'GQA') and any(k in n for k in head_keys):
            p.requires_grad = True
