...
def check_ans(options, ans, response):
    if len(ans) != 1:
        a = chr(ord('A') + options.index(ans)).lower()
    else:
        a = ans.lower()
...
def update_metrics_for_sample(metrics, sample, top_k, thres, cgbench_mode, conf_thr):
    ...
    # 2) QA metrics
    if 'answer' in sample or 'ans' in sample:
        for t in tasks:
            tab_ans[t][0] += 1
        tab_ans_all[0] += 1
...
def print_summary(metrics, top_k, thres, cgbench_mode):
    ...
    if cgbench_mode:
        ...
        headers += ['Acc (IoU >= 0.5)', 'Acc (IoP >= 0.5)']
    rows = []
    for task in tasks:
        if task not in tab_ans:
            continue
        total = tab_ans[task][0]
        failed = tab_ans[task][1]
        acc = tab_ans[task][2] / total * 100
        row = [task, total, failed, f'{acc:.2f}']
        if cgbench_mode:
            rec_iou = sum(tab_ans[task][i] / total for i in range(5, 5 + len(thres))) / len(thres) * 100
            row += [f'{rec_iou:.2f}']
        else:
            row += [f'{tab_ans[task][3] / total * 100:.2f}',
                    f'{tab_ans[task][4] / total * 100:.2f}']
        rows.append(row)

    # all
    total = tab_ans_all[0]
    failed = tab_ans_all[1]
    acc = tab_ans_all[2] / total * 100
    row_all = ['all', total, failed, f'{acc:.2f}']
    if cgbench_mode:
        rec_iou = sum(tab_ans_all[i] / total for i in range(5, 5 + len(thres))) / len(thres) * 100
        row_all += [f'{rec_iou:.2f}']
    else:
        row_all += [f'{tab_ans_all[3] / total * 100:.2f}',
                    f'{tab_ans_all[4] / total * 100:.2f}']
    rows.append(row_all)
