...
class AgentQwen2VLForConditionalGeneration(Qwen2VLForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        if self.config.role in ('all_in_one', 'grounder','GQA'):
            ...
            # 用于编码timestamps
            self.timestamp_embed = nn.Embedding(10000, config.hidden_size)
            self.spatial_merge_size = config.vision_config.spatial_merge_size
            self.timestamp_proj = nn.Linear(1, config.hidden_size)
    ...
    def prepare_inputs_for_generation(
        ...
        # 调用父类的prepare_inputs_for_generation函数
        ...
        # 添加自定义的timestamps_insert和duration
        model_inputs["timestamps_insert"] = kwargs.get("timestamps_insert", None)
        model_inputs["use_construct"] = kwargs.get("use_construct", False)
        model_inputs["use_duration"] = kwargs.get("use_duration", False)
        model_inputs["duration"] = kwargs.get("duration", None)
        model_inputs["use_qa"] = kwargs.get("use_qa", False)
    ...
    def forward(self,
        ...
        if use_construct and pixel_values_videos is not None and timestamps_insert is not None:
            video_embeds = self.visual(pixel_values_videos.type(self.visual.get_dtype()), grid_thw=video_grid_thw)
            if isinstance(timestamps_insert, list):
                timestamps_insert = torch.stack(timestamps_insert)
            timestamp_embeds = self.timestamp_embed(timestamps_insert.long())  # [t, hidden_size]

            num_frames = timestamps_insert.shape[0]
            # assert timestamps_insert.shape[0] == num_frames, "timestamps_insert doesn't match T"
            # assert video_embeds.shape[0] / self.spatial_merge_size ** 2 == num_frames, "video_embeds doesn't match T"

            video_and_time_embeds = []
            for i in range(num_frames):
                step = int(video_embeds.shape[0] / num_frames)
                for j in range(step):
                    video_and_time_embeds.append(video_embeds[step * i + j])  # 视频嵌入
                video_and_time_embeds.append(timestamp_embeds[i])  # 时间戳嵌入

            if use_duration and duration is not None:
                duration_embeds = self.model.embed_tokens(duration)
                video_and_time_embeds.append(duration_embeds)

            video_and_time_embeds = torch.stack(video_and_time_embeds, dim=0)  # 用stack而不是cat

            inputs_embeds = self.model.embed_tokens(input_ids)
            video_mask = (input_ids == self.config.video_token_id).unsqueeze(-1).expand_as(inputs_embeds)
            inputs_embeds = inputs_embeds.masked_scatter(video_mask, video_and_time_embeds.to(inputs_embeds.device,
                                                                                              inputs_embeds.dtype))
        ...
        if input_ids.max() > self.vocab_size:
            print("min/max token id:", input_ids.min().item(), input_ids.max().item())
            print("vocab size:", self.vocab_size)
            assert input_ids.max() > self.vocab_size, "input_ids.max() > self.vocab_size"
        ...
        if mode == 'training' and timestamps is not None:
            ...
            for batch_idx, (vision_inds, ts) in enumerate(zip(self.cache_vision_inds, timestamps)):
                ...
                for blk in self.vis_fuse:  # 28 * (self_attention+FFN)
            ...
            if logits.argmax() == self.config.reg_token_id:
                ...
                joint_tokens = torch.cat((vis_tokens + pe, reg_tokens), dim=1)  # visual+reg
                for blk in self.vis_fuse:  # 3* (self_attention+FFN)
                    joint_tokens = blk(joint_tokens)
                joint_tokens = self.vis_norm(joint_tokens)

                video_emb = joint_tokens[:, :-1]  # 这里的video_emb已经携带了reg的信息
                # video_emb: num_reg_tokens * num_frames * channel
...
# 将自定义配置类 AgentQwen2VLConfig 与一个标识模型类型的字符串（通常存储在 AgentQwen2VLConfig.model_type 中）关联起来
# 这样，当你调用 AutoConfig.from_pretrained(...) 时，框架会根据配置文件中的 model_type 自动返回 AgentQwen2VLConfig
AutoConfig.register(AgentQwen2VLConfig.model_type, AgentQwen2VLConfig)
# 将 AgentQwen2VLConfig 与对应的模型实现类 AgentQwen2VLForConditionalGeneration 关联起来
# 当你使用 AutoModel.from_pretrained(...) 加载模型时，框架会根据加载的配置自动实例化 AgentQwen2VLForConditionalGeneration 模型
