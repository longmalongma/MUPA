...
def build_model_multipath(model_path, config=None, is_trainable=False, merge_adapter=False, device='auto', dtype=torch.float16):
    # set do_resize to false to avoid duplicated resizing
    # https://github.com/huggingface/transformers/tree/main/src/transformers/models/qwen2_vl/image_processing_qwen2_vl.py
    processor = AutoProcessor.from_pretrained(model_path, do_resize=False)
    ...
    ground_adapter_path = nncore.join(model_path, 'grounder')
    ground_partial_path = nncore.join(model_path, 'pytorch_model.safetensors')
    if nncore.is_dir(ground_adapter_path) or nncore.is_file(ground_partial_path):
        print(f'Loading base model from {config.base_model_path}...')
        model = AutoModel.from_pretrained(
            config.base_model_path,
            config=config,
        ...
        if nncore.is_dir(ground_adapter_path):
            print('Initializing agent *grounder*')
            print(f'Loading adapter from {ground_adapter_path}...')
            # transformers integration does not support merge_and_unload, use peft instead
            model = PeftModel.from_pretrained(
                model,
                ground_adapter_path,
        ...
        if nncore.is_file(ground_partial_path):
            print(f'Loading state dict from {ground_partial_path}...')
            _, unexpected = load_model(model, ground_partial_path, strict=False, device=str(model.device))
            assert len(unexpected) == 0, f'unexpected parameters: {unexpected}'

        if merge_adapter and nncore.is_dir(ground_adapter_path):
            print('Merging adapter and unloading...')
            model = model.merge_and_unload()
            model._hf_peft_config_loaded = False
    else:
    ...
    for name in ['GQA', 'verifier', 'answerer']:
        path = nncore.join(model_path, name)
        if nncore.is_dir(path):
